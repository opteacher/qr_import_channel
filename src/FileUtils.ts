...
export const solitonDistributionK = Array.from(Array(solitonDistributionMax).keys())
  .map(i => [i + 1, solitonDistribution(i + 1, solitonDistributionMax)])
  .sort((a, b) => b[1] - a[1])
...
export const sampleSolitonDistributionK = (): number => {
  ...
  for (
    ;
    index < solitonDistributionKAggregated.length &&
    solitonDistributionKAggregated[index][1] >= randValue;
    index++
  ) {}
...
export const marshalDescriptor = (descriptor: Descriptor, nominalSliceSize: number) => {
  ...
  const prePadded =
    'D:' + b64encode(encoder.encode(JSON.stringify(descriptor)), undefined, undefined)
  if (prePadded.length < marshaledNominalSlizeSize(nominalSliceSize)) {
    ...
    return (
      'D:' +
      b64encode(
        encoder.encode(JSON.stringify({ ...descriptor, padding: padding })),
        undefined,
        undefined
      )
    )
...
export const marshalSlice = async (file: File, sliceSize: number, sliceIdentifiers: number[]) => {
  ...
  if (marshaled.length < marshaledNominalSlizeSize(sliceSize)) {
    return (
      marshaled + ':' + Array(marshaledNominalSlizeSize(sliceSize) - marshaled.length - 1).join('x')
    )
  } else {
    return marshaled
  }
...
export const uniqueIdentifiers = (a: number[], b: number[]): number[] => {
  return [...a, ...b]
    .filter(e => {
      return (
        (a.indexOf(e) === -1 && b.indexOf(e) !== -1) || (a.indexOf(e) !== -1 && b.indexOf(e) === -1)
      )
    })
    .sort()
}
...
export const determinedSliceIndices = (store: SliceStore): number[] => {
  return store
    .map((slice, index) => {
      return { s: slice, i: index } as { s: Slice; i: number }
    })
    .filter(v => v.s.identifiers.length !== 0)
    .map(v => v.i)
}
...
export const assemblePayload = (store: SliceStore, descriptor: Descriptor): Blob | null => {
  if (isDetermined(store)) {
    ...
    store
      .filter(s => s.identifiers.length === 1)
      .sort((a, b) => {
